...
const OPERATION_EMAIL = "operation@modernsmart.com"; // this should be the operation account
...
class AttendanceBot {
    ...
    async joinSession(title, meetLink, thread) {
        ...
        while (await this.isInMeet() === false && count < 7) {

            try {

                const joinButton = await page.waitForSelector('button[data-idom-class="nCP5yc AjY5Oe DuMIQc LQeN7 jEvJdc QJgqC"][jsname="Qx7uuf"]', { visible: true, timeout: 10 * 1000 });
    ...
    async takeAttendance(thread, interval, guestList, eventInfo, fileName) {
        ...
        if ((after6mins < timeRightNow && timeRightNow < after7mins) ||
            (after16mins < timeRightNow && timeRightNow < after17mins)) {
        ...
        if (timeRightNow < after30mins) {
            ...
            for (const absentee of newUniqueMissing) {
                console.dir(absentee, { depth: null });
                ...
                if (absentee.role === 'tutor') {
                    if (timeRightNow > after3mins && timeRightNow < after4mins) {
                        ...
                        if (presentStudents.length > 0) {
                            ...
                            const notificationSlackPresentStudent = `:incoming_envelope: Notification sent to ${presentStudentName}`
                            sendingEmailPresentUser(absenteeName, presentStudentEmails);
                    ...
                    if (timeRightNow > after5mins && timeRightNow < after6mins) {
                        ...
                        if (presentTutors.length > 0) {
                            ...
                            sendingEmailPresentUser(absenteeName, presentTutorEmails);
                            const notificationSlackPresentTutor = `:incoming_envelope: Notification sent to ${presentTutorNames}`
                ...
                if (presentUserEmail) {
                    sendingEmailAbsent(eventInfo, presentUserEmail);
                    let notificationSlackAbsentee = `:incoming_envelope: Reminder sent to ${absenteeName}`;
...
async function sendEmail(auth, to, cc, subject, body, isHtml = false) {
    const gmail = google.gmail({ version: 'v1', auth });
...
async function sendingEmailAbsent(event, email) {
    try {
        const auth = await authorize();
        ...
        const ccAddress = process.env.NOTIFICATION_EMAIL;
        const emailSubject = `Please Join Now: ${eventTitle}`;
        const emailBody = `
        ...
        await sendEmail(auth, toAddress, ccAddress, emailSubject, emailBody, true);
    } catch (error) {
        console.error(error);
    }
}
